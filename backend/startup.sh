...
PYTHON_PATH="$APP_DIR/antenv/bin/python"

# Build the cron entry
echo "30 9 * * 1-5 cd $APP_DIR && $PYTHON_PATH manage.py daily_update --settings=config.settings.production >> $CRON_LOG 2>&1" > /etc/cron.d/daily-update
